...
test("normalizeWord는 헷갈리는 유의어 필드를 표준 배열로 변환한다", () => {
	const word = normalizeWord({
		term: "資料",
	...
	assert.deepEqual(word.confusingSynonyms, ["材料", "データ"]);
});
...
test("getSearchText와 filterWords는 헷갈리는 유의어 검색을 지원한다", () => {
	const words = [
		{ term: "配布", pronunciations: ["はいふ"], meanings: ["배포"], examples: [], confusingSynonyms: ["配る"] },
		{ term: "資料", pronunciations: ["しりょう"], meanings: ["자료"], examples: [], confusingSynonyms: ["材料"] },
	];

	assert.equal(getSearchText(words[0], "all").includes("配る"), true);
	assert.deepEqual(filterWords(words, { query: "材料", scope: "synonym" }).map((word) => word.term), ["資料"]);
});
